...
| Fitting-room privacy fix | Face/body uploads (`/api/fitting-room/photo`) were going through the same `_save_upload()` as wardrobe items, landing in the git-tracked `data/photos/`. Split a gitignored `data/personal_uploads/` (served via `/personal-photos/{name}` with `Cache-Control: no-store`) so personal photos can never be committed, per the never-save-without-consent design. Confirmed `data/photos/` held no personal photos (all 52 tracked files are wardrobe items/cutouts) | Design intent was load-bearing here — separated the private upload path before letting a real face/body photo anywhere near the repo |
| IDM-VTON live-test gap | Verified the multi-mirror client only exercises `yisol/IDM-VTON` — `kadirnar/IDM-VTON` was down (`RUNTIME_ERROR`, no queue) at test time, so the failover branch has never actually been hit. Also hit anonymous ZeroGPU quota ("exceeded ZeroGPU quota, 60s vs 88s left") — inference reaches `FINISHED` but output download fails without an `HF_TOKEN`. Added optional `HF_TOKEN` passthrough to `vton_client.py` | Known gap, not urgent: failover logic is untested, and anonymous users get roughly one try-on before throttling. Set `HF_TOKEN` for real use |
| Quiz v2 built | 18-question experience: Section 1 (wardrobe pairs from real items), Section 2 (scenario-based using real wardrobe via RAG), Section 3 (shopping instincts). Live 3-axis style meter (casual/formal, minimal/maximal, timeless/trendy) updates after each answer. LLM synthesis (Gemini structured output) produces named personality + gap-based shopping recs grounded in real wardrobe DNA. Result connects directly to existing Should I Buy This? flow. Client-side html2canvas export for the personality card. Retake mechanic with 30-day cadence | Grounds the personality/recs in `get_wardrobe_dna()` (real counts, missing/underrepresented categories) rather than vibes. Verified backend acceptance (DNA + analyze endpoints return real 50-item breakdown and a named personality); frontend target-confirmed against the real wardrobe before styling |
...
- Quiz v2 manual QA: run the full 18-question flow against the live backend (deck builds from real photos, meter animates, analyze returns a personality, save-as-image produces a PNG, retake cadence behaves). Worth a real-device pass at 390/768/1024px before calling it shipped.
